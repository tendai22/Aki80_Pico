...
<img width=500 src="img/002-Aki80_Pico_Board.png"/>
...
* HALT, RESET に LED を繋ぎ状態が見えるようにする。
...
* HALT(黄), RESET(緑)に抵抗入りLEDを繋ぐ。Low で点灯するように上から吊る。
...
WE をネゲートした瞬間に OE のアサートの効果が出て、SRAM の出力バッファが有効になる。Pico のデータバス出力と同時に出力することになるが、幸いにして、Pico の出力ビットパターンと SRAM の出力ビットパターンは同じである。つまり Pico が 1 を出力しているときは、SRAM も 1 を出力する。 0 の時は 0 を出力している。よって、一方が High, 他方が Low になることはない。それぞれの出力電圧に違いはあるが、多少はましであろう。

うーん、完全ではないので、WE が OE に勝つ作戦はいったんおいておくことにしよう。命令置きで進める。

## プログラム実行中の Pico の処理

UART エミュレーションの I/O ポートレジスタを 20H, 21H に割り当てる。ここへの I/O マシンサイクルは WAIＴｅＴｅT を掛けて引き延ばし、Pico の UART の状態から Z80 に見せるステータスレジスタのビットパターンを作成する。受信データを読み出して Z80 に返す、または、Z80 から　OUT されたデータを UART 送信する。
...
* まず、Z80 Free Run する。WAIT == High, RAMCS2, RAMWE == High にしておいて、RESET を High にして MREQ, RFSH, RD の波形を見る。
* ブートローダを作成する。命令置きで SRAM 書き込みと読み出しを作る。書き込んだデータが読み出せることを確認。PIO を使って WAIT 制御して、MREQ 終了時のデータバス Hi-Z 切り替えに要する時間を確認しておく。間に合わなければ BUSRQ で止める処理を入れる。
* 18,FEを置いて Z80 start, アドレスバスも含めた波形を見る。
* Aki80 BASIC をロードして動くかどうかを見る。

## Aki80 用 BASIC

電脳伝説さんのリポジトリから [Aki-80 用 BASIC](https://github.com/vintagechips/aki80basic.git)を入手しておく。

解説は、彼の[ブログ記事](https://vintagechips.wordpress.com/2018/09/10/aki-80_basic/)を参照したい。Aki80_Pico の回路図にも配線を反映してある。

Hex ファイルがあるので、C言語配列に変換しておいた。変換手順とツールは本フォルダ直下の `basic` 直下にある。
